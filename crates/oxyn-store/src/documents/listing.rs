    ...
    pub fn page(
        ...
        self.store.with_connection_cancellable(cancel, |connection| {
            ...
            let mut entries = query.query_and_then(params![workspace.to_string(), filter.saved_only, filter.open_only, search, filter.before.map(|id| id.to_string()), i64::from(filter.limit)+1], |row| -> Result<DocumentSummary> {
                Ok(DocumentSummary { id: parse_id(&row.get::<_, String>(0)?, "documents.id")?, title: row.get(1)?, connection: parse_id_opt(row.get(2)?, "documents.connection_id")?, connection_name: row.get(3)?, updated_at: row.get(4)?, is_saved: row.get(5)?, is_open: row.get(6)?, has_changes: row.get(7)? })
