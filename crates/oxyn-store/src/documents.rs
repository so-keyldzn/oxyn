...
impl<'a> Documents<'a> {
    ...
    pub fn get(&self, id: DocumentId) -> Result<Option<Document>> {
        self.store.with_connection(|conn| {
            conn.query_row(
                &format!("{SELECT_COLONNES} WHERE id = ?1"),
                params![id.to_string()],
    ...
    pub fn list(&self, workspace: WorkspaceId) -> Result<Vec<Document>> {
        self.store.with_connection(|conn| {
            let mut requete = conn.prepare(&format!(
                "{SELECT_COLONNES} WHERE workspace_id = ?1 ORDER BY updated_at DESC, id"
            ))?;
            requete
...
const SELECT_COLONNES: &str = "SELECT id, workspace_id, title, language, content, \
     connection_id, created_at, updated_at FROM documents";
